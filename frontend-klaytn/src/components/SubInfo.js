...
export default ({ tokenId, D_day }) => {
  ...
  <Container>
    <ColoredBox>
      ...
      <Description>
        목표 KLAY가 100% 이상 모이면 메이커스가 성공되는 프로젝트입니다!<br />
        이 프로젝트는 마감일까지 목표 금액이 100% 모이지 않으면
        결제가 진행되지 않습니다.<br />
        에코 메이커스의 멋진 아이디어가 세상에 알려질 수 있도록 SNS에 공유해주세요.<br />
        SNS 공유 이벤트 참여시 추첨을 통해 최대 100 ECO를 적립해 드립니다.<br />
